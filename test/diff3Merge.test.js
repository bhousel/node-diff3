...
test('diff3Merge', function(t) {
  ...
  t.test('strings can optionally split on given separator', function(t) {
    ...
    const res = Diff3.diff3Merge(a, o, b, { stringSeparator: ', ' });

    t.same(res[0].ok, ['new hampshire']);
    t.same(res[0].conflict, undefined);

    t.same(res[1].ok, undefined);
    t.same(res[1].conflict.o, ['new mexico']);
    t.same(res[1].conflict.a, ['new jersey']);
    t.same(res[1].conflict.b, ['new york']);

    t.same(res[2].ok, ['north carolina']);
    t.same(res[2].conflict, undefined);
